...
export function DraggableEvent({ event, showTime, compact, onClick }: DraggableEventProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        'text-sm text-primary-foreground mb-0.5 cursor-grab active:cursor-grabbing transition-all duration-300 ease-out truncate whitespace-nowrap overflow-hidden min-w-0 opacity-80',
        compact ? 'px-1.5 py-0.5 text-xs sm:text-sm sm:px-2 sm:py-1' : 'px-2.5 py-1.5',
