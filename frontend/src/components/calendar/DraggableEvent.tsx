...
export function DraggableEvent({ event, showTime, compact, onClick }: DraggableEventProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        'text-xs text-primary-foreground mb-0.5 cursor-grab active:cursor-grabbing transition-all duration-300 ease-out truncate whitespace-nowrap overflow-hidden min-w-0',
        compact ? 'px-1 py-0.5 text-[11px] sm:text-xs sm:px-1.5 sm:py-1' : 'px-2 py-1',
        eventColors[event.color] || 'bg-blue-500',
        isDragging ? 'opacity-30 scale-105 shadow-xl z-50 ring-2 ring-primary/20' : 'hover:scale-[1.02] hover:shadow-md'
      )}
    >
      {showTime && event.time && (
